...
<div
    @class([
        ...
        'bg-deep border-zinc-800 text-white' => $dark,
    ])
    style="min-height: 160px;"
>
    @if ($type === 'pricing')
        <div class="mb-3 flex items-center gap-2">
            <div class="h-2 w-2 rounded-full {{ $dark ? 'bg-zinc-300' : 'bg-deep' }}"></div>
        ...
        <div class="h-8 w-full rounded-lg {{ $dark ? 'bg-white' : 'bg-deep' }}"></div>

    @elseif ($type === 'hero')
        <div class="mb-3 h-3 w-3/4 rounded {{ $dark ? 'bg-white' : 'bg-deep-2' }}"></div>
        <div class="mb-4 h-3 w-1/2 rounded {{ $dark ? 'bg-zinc-500' : 'bg-deep-2' }}"></div>
        <div class="mb-4 space-y-2">
            <div class="h-2 w-full rounded {{ $dark ? 'bg-zinc-700' : 'bg-zinc-200' }}"></div>
            <div class="h-2 w-5/6 rounded {{ $dark ? 'bg-zinc-700' : 'bg-zinc-200' }}"></div>
            <div class="h-2 w-4/5 rounded {{ $dark ? 'bg-zinc-700' : 'bg-zinc-200' }}"></div>
        </div>
        <div class="h-8 w-28 rounded-lg bg-deep {{ $dark ? 'bg-white' : '' }}"></div>
    ...
    @elseif ($type === 'testimonial')
        <div class="mb-3 flex gap-1">
            @for ($i = 0; $i < 5; $i++)
                <div class="h-3 w-3 rounded-sm bg-deep"></div>
            @endfor
        </div>
    ...
    @elseif ($type === 'thumbnail')
        ...
        <div class="mb-2 h-2 w-3/4 rounded bg-deep-2"></div>
        <div class="h-2 w-full rounded bg-zinc-200"></div>
    ...
    @elseif ($type === 'badge')
        ...
        <div class="mb-3 h-3 w-2/3 rounded bg-deep-2"></div>
        <div class="space-y-1.5">
            <div class="h-2 w-full rounded bg-zinc-200"></div>
            <div class="h-2 w-4/5 rounded bg-zinc-200"></div>
        </div>
        <div class="mt-3 h-8 w-24 rounded-lg bg-deep"></div>

    @elseif ($type === 'form')
        <div class="mb-4 h-2 w-1/2 rounded bg-deep-2"></div>
        <div class="space-y-3">
        ...
        <div class="mt-3 h-8 w-full rounded-lg bg-deep"></div>

    @elseif ($type === 'feature')
        <div class="mb-3 inline-flex h-9 w-9 items-center justify-center rounded-xl {{ $dark ? 'bg-deep-2' : 'bg-zinc-100' }}">
            <div class="h-4 w-4 rounded {{ $dark ? 'bg-white' : 'bg-deep' }}"></div>
        </div>
        <div class="mb-2 h-3 w-2/3 rounded {{ $dark ? 'bg-white' : 'bg-deep-2' }}"></div>
        <div class="space-y-1.5">
            <div class="h-2 w-full rounded {{ $dark ? 'bg-zinc-700' : 'bg-zinc-200' }}"></div>
            <div class="h-2 w-5/6 rounded {{ $dark ? 'bg-zinc-700' : 'bg-zinc-200' }}"></div>
        </div>
