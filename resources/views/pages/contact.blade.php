    ...
    <section id="form" class="scroll-mt-20 bg-white py-16 lg:py-24">
        <div class="mx-auto grid max-w-6xl grid-cols-1 gap-10 px-4 sm:px-6 lg:grid-cols-5 lg:gap-16 lg:px-8">
            ...
            <div class="lg:col-span-2">
                ...
                <div class="mt-6 space-y-3">
                    ...
                    <a href="mailto:{{ $company['email'] }}" class="flex items-center gap-4 rounded-2xl border border-zinc-200 p-4 transition hover:border-zinc-400 hover:bg-zinc-50">
                        <span class="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-zinc-900 text-white">
                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.7" d="M3 8l9 6 9-6M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                        </span>
                        <span>
                    ...
                    <a href="tel:{{ $phoneHref }}" class="flex items-center gap-4 rounded-2xl border border-zinc-200 p-4 transition hover:border-zinc-400 hover:bg-zinc-50">
                        <span class="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-zinc-900 text-white">
                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.7" d="M3 5a2 2 0 012-2h2.6a1 1 0 01.95.68l1.2 3.5a1 1 0 01-.5 1.2L8 9.6a12 12 0 006.4 6.4l1.2-1.5a1 1 0 011.2-.5l3.5 1.2a1 1 0 01.68.95V19a2 2 0 01-2 2A16 16 0 013 5z" /></svg>
                        </span>
                        <span>
                ...
                <div class="mt-3 flex gap-3">
                    @foreach ($company['social'] as $network => $url)
                        <a href="{{ $url }}" target="_blank" rel="noopener noreferrer" aria-label="{{ ucfirst($network) }}" class="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-zinc-200 text-zinc-700 transition hover:border-zinc-900 hover:text-ink">
                            @switch($network)
                                @case('facebook') <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24"><path d="M13.5 9H16l.5-3h-3V4.5c0-.87.3-1.5 1.6-1.5H16V.3C15.7.27 14.8.2 13.8.2 11.6.2 10 1.6 10 4.1V6H7v3h3v9h3.5z" /></svg> @break
                                @case('instagram') <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="5" stroke-width="1.7"/><circle cx="12" cy="12" r="4" stroke-width="1.7"/><circle cx="17.5" cy="6.5" r="1" fill="currentColor" stroke="none"/></svg> @break
                ...
                {{-- Alternativă ghidată --}}
                <div class="relative overflow-hidden mt-8 rounded-2xl bg-zinc-900 p-6 text-white">
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 gap-10 lg:grid-cols-3 lg:gap-16">
                ...
                <div class="lg:col-span-1">
                    ...
                    <div class="relative overflow-hidden mt-8 rounded-3xl bg-zinc-900 p-6 text-white">
                        <div class="bg-dot-grid pointer-events-none absolute inset-0 opacity-[0.12]"></div>
                        <div class="relative">
                        ...
                        @foreach (__('pages.contact.faq') as $i => $item)
                            <div>
                                <button type="button" @click="open === {{ $i }} ? open = null : open = {{ $i }}" class="flex w-full items-center justify-between gap-4 px-6 py-5 text-left transition hover:bg-zinc-50" :aria-expanded="open === {{ $i }} ? 'true' : 'false'">
                                    ...
                                    <svg class="h-5 w-5 shrink-0 text-zinc-900 transition-transform duration-200" :class="open === {{ $i }} ? 'rotate-45' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v14M5 12h14" /></svg>
                                </button>
